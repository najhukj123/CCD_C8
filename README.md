...
这是一个 STM32F103C8 小车工程。线性 CCD 模块每帧提供 1500 个灰度像素；固件取中央的 1000 点找赛道，再用编码器反馈调节两路 PWM。电脑端通过串口发送命令并显示数据。当前固件和电脑端统一使用 **921600 8N1**。
...
**上赛道前的 CCD 标定：**把车停在普通循迹线上，先看综合调试台是否显示 `TRACKING`，再点“当前位置设为中心”。白色虚线表示第 499.5 个像素的数值中点，不保证等于镜头实际正前方；紫色虚线才是标定后的转向零点。找线范围以这个零点为中心，允许首次标定前偏离数值中点约 300 像素。若单片机仍报 `LOST`，调试台会用橙色显示它从同一帧找到的本地候选线；橙色只帮助排查，不能替代单片机的 `TRACKING` 状态。蓝色曲线始终按收到的原始像素位置绘制，不做平移。

**CCD 像素窗口：**原生 CCD 模块经 COM14 实测每帧为 10 字节帧头加 1500 个像素。旧固件只读帧头后的前 1000 点，又把它左右镜像，因此原生画面第 750 点的黑线会落在调试台约第 249 点。现在 `_ccd_sensor.c` 先读取完整帧，截取中间的 1000 点（原生第 250～1249 点），再镜像并发送给调试台。这样原生画面正中的黑线应落在调试台约第 500 点。若更改模块的“使用长度”，需要先确认固件的 `CCD_MODULE_PIXEL_COUNT` 上限及显示窗口。
