...
找线时，固件从左边向中间找第一个低于阈值的黑线像素作为左边界，从右边向中间同样找右边界；白线模式则找第一个高于或等于阈值的像素。传感器边缘的暗区会先跳过，边界外侧需有至少 3 个背景像素。中心取 `(左边界 + 右边界) / 2`，原有线宽、中心偏移和跳变限制仍生效。转向使用 **PD**：`转向量 = Kp × 当前偏差 + Kd × (当前偏差 - 上次偏差)`。右轮目标是 `基础RPM - 转向量`，左轮目标是 `基础RPM + 转向量`。每个车轮再单独用速度 **PID** 跟随自己的目标 RPM。
...
**调试台的帧数：**CCD 序号每次采集都会增加，固件约每 80 毫秒才上传一次；两个上传报文之间的序号间隔不代表蓝牙丢包。调试台现在分别显示收到的 CCD 报文数、全 7 异常帧数、界面略过的帧数和 CRC 错误。COM18 曾实测收到 CRC 正确、但 1000 个像素全部为 7 的异常帧；调试台保留上一张有效波形，固件也跳过这种帧，不让它参与循迹。
整圈记录 CSV 中的旧列名 `ccd_dropped_frames` 为兼容已有图表而保留，现在表示界面略过的有效帧数，不表示蓝牙传输丢包。
